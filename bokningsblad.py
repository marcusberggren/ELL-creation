...
def update_data_sheet():
    ...
    data_df['MLO_check'] = fn.MLO_check(df)
    data_df['terminal_check'] = fn.terminal_check(df)
    data_df['cargo_type_check'] = fn.cargo_type_check(df)
    data_df['container_check'] = df['CONTAINER'].apply(fn.container_check, 1)
    data_df['load_status_check'] = fn.load_status_check(df)
